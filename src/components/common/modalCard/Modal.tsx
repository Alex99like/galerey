...
const Modal: FC<IPropsModal> = ({ card, closeModal }) => {
  ...
  const { cover_photo, preview_photos, user, tags } = card;
  return (
    <div
      className={`background-modal ${activeModal ? 'active' : ''}`}
      onClick={() => closeModal(false)}
    >
      <div className={`modal ${activeModal ? 'active' : ''}`} onClick={(e) => e.stopPropagation()}>
